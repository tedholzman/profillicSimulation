...
#include "Profile.hpp"
#include "Fasta.hpp"
#include "ProfuseTest.hpp" // for the parameters
...
setTransitionsFromParameters (
  typename ProfuseTest<ResidueType, ProbabilityType, ScoreType, MatrixValueType, SequenceResidueType>::Parameters const & parameters,
  ProfileType & profile
)
{
...
} // setTransitionsFromParameters( ProfuseTest::Parameters const &, ProfileType & profile )
...
)
{
  ...
  setTransitionsFromParameters<ResidueType, ProbabilityType, ScoreType, MatrixValueType, SequenceResidueType, ProfileType>(
    parameters,
    profile
  );
  ...
  // Use the first element of the conservationRates parameter vector, or .75 if
  // it is null.
  double conservation_rate =
    (
      parameters.conservationRates ?
      ( *parameters.conservationRates )[ 0 ] :
      .75
    );
  ...
  // That's it.
  return;
} // consensusToProfile( ProfuseTest::Parameters const &, Sequence const &, ProfileType & )
...
main ( int const argc, char const ** argv )
{
  ...
  galosh::ProfuseTest<seqan::Dna, floatrealspace, floatrealspace, floatrealspace, seqan::Dna>::Parameters parameters;

  if( argc >= 3 ) {
    // arg 3 is the conservation rate (default .75)
    vector<double> conservation_rates( 1 );
    parameters.conservationRates = &conservation_rates;
    try {
      conservation_rates[ 0 ] = boost::lexical_cast<double>( argv[ 3 ] );
    ...
    if( conservation_rates[ 0 ] <= 0 ) {
      std::cerr << "The given conservation rate value, " << conservation_rates[ 0 ] << ", is zero or negative.  You must supply a value between 0 and 1, and not 0." << std::endl;
      exit( 1 );
    }
    if( conservation_rates[ 0 ] > 1 ) {
      std::cerr << "The given conservation rate value, " << conservation_rates[ 0 ] << ", is greater than 1.  You must supply a value between 0 and 1." << std::endl;
      exit( 1 );
    }
  } // End if argc >= 3

  galosh::ProfileTreeRoot<seqan::Dna, floatrealspace> profile;
  galosh::consensusToProfile<seqan::Dna, floatrealspace, floatrealspace, floatrealspace, seqan::Dna, galosh::ProfileTreeRoot<seqan::Dna, floatrealspace> >( parameters, fasta[ 0 ], profile );
